...
public class TicketReservationConsumer : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.ReceivedAsync += async (_, eventArgs) =>
        {
            ...
            try
            {
                var message = JsonSerializer.Deserialize<ReservationMessageDto>(json, new JsonSerializerOptions
                {
                ...
                if (message is not null)
                {
                    ...
                    var reserveTicketUseCase = scope.ServiceProvider.GetRequiredService<IReserveTicketUseCase>();

                    await reserveTicketUseCase.ExecuteAsync(message, stoppingToken);
                }
